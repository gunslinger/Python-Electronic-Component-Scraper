...
class FarnellSpider(Spider):
    ...
    def __init__(self, query='', *args, **kwargs):
        ...
        self.start_urls = ['http://uk.farnell.com']
        self.query = query
        self.debug = p_setting.get("APP_DEBUG")
        self.spider_name = "Farnell"
    ...
    def start_requests(self):
        ...
        Returns:
            response
        """
        yield Request('http://uk.farnell.com/w/search/prl/results?st=%s' % self.query, callback=self.parse_search)
    ...
    def parse_search(self, response):
        ...
        search_found = s.css("span#titleProdCount::text").extract_first()
    ...
    def single_result(self, response):
        ...
        part_number = cleansplit(Selector(text=response.body)
                                    .css('dd[itemprop="http://schema.org/sku"]::text').extract_first())
        ...
        manufacturer_name = cleansplit(Selector(text=response.body)
                                    .css('span[itemprop="http://schema.org/manufacturer"]::text').extract_first())
        ...
        image_url = cleansplit(Selector(text=response.body)
                                    #.xpath("/img[@id='productMainImage']/@src").extract_first())
    ...
    def parse_search_result(self, response):
        ...
        for i, j, k, l, m, n in zip(part_number, manufacturer_part_number, manufacturer_name,
            ...
            yield item
        next_url = response.xpath("//span[@class='current']"
